...
const HOSTED_ZONE_NAME = 'production.com';
...
const result = [
    ...
    { weightage: 0, name: `The record with 'hostid' is available`, status: false, error: '' },
    { weightage: 0, name: `The 'hostid' record is of type 'PTR'`, status: false, error: '' },
];
...
async function checkHostedZone() {
  try {
    ...
    if (hostedZone) {
      ...
      const hostnameRecord = resourceRecordSetsData.ResourceRecordSets.find(r => r.Name === 'hostid.production.com.');
      try {
        if (hostnameRecord) {
          result[2].status = true;
          result[2].weightage = 0.25;
          try {
            if (hostnameRecord.Type === 'PTR') {
              result[3].status = true;
              result[3].weightage = 0.25;
            }
            else {
              result[3].error = 'Hostid record does not exist to validate PTR record type';
            }
          }
          catch (error) {
            result[3].error = 'Hostid record does not exist to validate PTR record type';
          ...
          result[2].error = 'Hostid record does not exist';
          result[3].error = 'Hostid record does not exist to validate PTR record type';
        }
      }
      catch (error) {
