...
const HOSTED_ZONE_NAME = 'iamneo.com';
...
const result = [
  ...
  { weightage: 0, name: `The record with 'hostname' is available`, status: false, error: '' },
  { weightage: 0, name: `The 'hostname' record is of type 'TXT'`, status: false, error: '' },
];
...
async function checkHostedZone() {
  try {
    ...
    if (hostedZone) {
      ...
      const hostnameRecord = resourceRecordSetsData.ResourceRecordSets.find(r => r.Name === 'hostname.iamneo.com.');
      try {
        if (hostnameRecord) {
          result[2].status = true;
          result[2].weightage = 0.25;
          try {
            if (hostnameRecord.Type === 'TXT') {
              result[3].status = true;
              result[3].weightage = 0.25;
            }
            else {
              result[3].error = 'Hostname record does not exist to validate TXT record type';
            }
          }
          catch (error) {
            result[3].error = 'Hostname record does not exist to validate TXT record type';
          ...
          result[2].error = 'Hostname record does not exist';
          result[3].error = 'Hostname record does not exist to validate TXT record type';
        }
      }
      catch (error) {
