...
const HOSTED_ZONE_NAME = 'dev-test.com';
...
const result = [
  ...
  { weightage: 0, name: `The record with 'testing' is available`, status: false, error: '' },
  { weightage: 0, name: `The 'testing' record is of type 'A'`, status: false, error: '' },
];
...
async function checkHostedZone() {
  try {
    ...
    if (hostedZone) {
      ...
      const hostnameRecord = resourceRecordSetsData.ResourceRecordSets.find(r => r.Name === 'testing.dev-test.com.');
      try {
        if (hostnameRecord) {
          result[2].status = true;
          result[2].weightage = 0.25;
          try {
            if (hostnameRecord.Type === 'A') {
              result[3].status = true;
              result[3].weightage = 0.25;
            }
            else {
              result[3].error = 'Testing record does not exist to validate A record type';
            }
          }
          catch (error) {
            result[3].error = 'Testing record does not exist to validate A record type';
          ...
          result[2].error = 'Testing record does not exist';
          result[3].error = 'Testing record does not exist to validate A record type';
        }
      }
      catch (error) {
