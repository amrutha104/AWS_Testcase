...
const result = [
  { weightage: 0, name: "'public-vpc' is available", status: false, error: '' },
  { weightage: 0, name: "VPC CIDR is '10.10.0.0/16'", status: false, error: '' },
  { weightage: 0, name: "'us-east-1a' has a subnet with the CIDR '10.10.1.0/24'", status: false, error: '' },
  { weightage: 0, name: "'us-east-1b' has a subnet with the CIDR '10.10.2.0/24'", status: false, error: '' },
  { weightage: 0, name: "Internet gateway is attached to 'public-vpc'", status: false, error: '' },
  { weightage: 0, name: "Internet gateway is added in the route table", status: false, error: '' },
];

async function checkVPC() {
  const ec2 = new EC2({ region: 'us-east-1', credentials });

  try {
    // Describe VPC
    const vpcData = await ec2.describeVpcs({
      Filters: [
        { Name: 'tag:Name', Values: ['public-vpc'] },
        { Name: 'cidr', Values: ['10.10.0.0/16'] },
      ],
    });

    if (vpcData.Vpcs && vpcData.Vpcs.length > 0) {
      for (const vpc of vpcData.Vpcs) {
        const vpcNameTag = vpc.Tags.find(tag => tag.Key === "Name" && tag.Value === "public-vpc");

        if (vpcNameTag) {
          result[0].weightage = 0.1;
          result[0].status = true;
        } else {
          result[0].error = "VPC 'public-vpc' is not available";
        }

        try {
          if (vpc.CidrBlock === '10.10.0.0/16') {
            result[1].weightage = 0.1;
            result[1].status = true;
          } else {
            result[1].error = "VPC CIDR is not '10.10.0.0/16'";
          }
        } catch (error) {
        ...
        try {
          ...
          if (subnets.Subnets && subnets.Subnets.length > 0) {
            const subnet1 = subnets.Subnets.find(a => a.CidrBlock === '10.10.1.0/24' && a.AvailabilityZone === 'us-east-1a');
            const subnet2 = subnets.Subnets.find(b => b.CidrBlock === '10.10.2.0/24' && b.AvailabilityZone === 'us-east-1b');
            ...
            try {
              if (subnet1) {
                ...
                result[2].error = "Subnet 'us-east-1a' with CIDR '10.10.1.0/24' is not available";
              }
            } catch (error) {
              result[2].error = "VPC 'public-vpc' is not available to validate subnet 'us-east-1a' with CIDR '10.10.1.0/24'";;
            }
            ...
            try {
              if (subnet2) {
                ...
                result[3].error = "Subnet 'us-east-1b' with CIDR '10.10.2.0/24' is not available";
              }
            } catch (error) {
              result[3].error = "Error checking subnet 'us-east-1b': " + error.message;
            }
          } else {
  ...
  result[5].error = "VPC 'public-vpc' is not available to validate Internet gateway is added in the route table";
}

            } else {
              result[4].error = "Internet gateway is not attached to 'public-vpc'";
              result[5].error = "Internet gateway is not added in the route table";
            }
          } else {
          ...
          result[4].error = "VPC 'public-vpc' is not available to validate Internet gateway is attached to 'public-vpc'";
          result[5].error = "VPC 'public-vpc' is not available to validate Internet gateway is added in the route table";
        }
      }
    } else {
      ...
      result[1].error = "VPC CIDR is not '10.10.0.0/16'";
      result[2].error = "VPC 'public-vpc' is not available to validate subnet 'us-east-1a' with CIDR '10.10.1.0/24'";
      result[3].error = "VPC 'public-vpc' is not available to validate subnet 'us-east-1b' with CIDR '10.10.2.0/24'";
      result[4].error = "VPC 'public-vpc' is not available to validate Internet gateway is attached to 'public-vpc'";
      result[5].error = "VPC 'public-vpc' is not available to validate Internet gateway is added in the route table";
    }
