...
const result = [
        ...
        { name: "The period given is '6 Hour'", weightage: 0, status: false, error: '' },
...
async function validateCloudWatchAlarms(){
    ...
    try {
        ...
        if (data.MetricAlarms.some(alarm => alarm)) {
            ...
            try {
                if (data.MetricAlarms.some(alarm => alarm.Period === 21600)) {
                    result[2].status = true;
                    result[2].weightage = 0.25;
                } else {
                    result[2].error = "Alarm does not have the expected Period of 6 Hour (21600 seconds).";
                }
            } catch (error) {
                result[2].error = "Alarm does not have the expected Period of 6 Hour (21600 seconds).";
            }
