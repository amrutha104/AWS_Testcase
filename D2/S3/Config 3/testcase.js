...
const region = 'ca-central-1'; // Change the region if required

const tagkey = 'Bucketlogging';
const tagvalue = 'Enabled';
const configrulename = 'Devtest';
const validationResult = [
    { weightage: 0, name: "ConfigRule is named as 'Devtest'", status: false, error: '' },
    { weightage: 0, name: "Assigned ConfigRule is 'S3_BUCKET_LOGGING_ENABLED'", status: false, error: '' },
...
async function validateAWSConfigRules() {
    try {
        ...
        if (testUserRule) {
            ...
            validationResult[0].error = "ConfigRule 'Devtest' not found.";
        }

        try {
            if (testUserRule && testUserRule.Source.SourceIdentifier === 'S3_BUCKET_LOGGING_ENABLED') {
                validationResult[1].weightage = 0.25;
                validationResult[1].status = true;
            } else {
                validationResult[1].error = "Assigned ConfigRule is not 'S3_BUCKET_LOGGING_ENABLED'.";
            }
        } catch (error) {
            validationResult[1].error = "Assigned ConfigRule is not 'S3_BUCKET_LOGGING_ENABLED'.";
        }
