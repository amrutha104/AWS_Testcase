...
const tagkey = 'Bucket';
const tagvalue = 'Versioning';
const configrulename = 'Testuser';
const validationResult = [
    { weightage: 0, name: "ConfigRule is named as 'Testuser'", status: false, error: '' },
    { weightage: 0, name: "Assigned ConfigRule is 'S3_BUCKET_VERSIONING_ENABLED'", status: false, error: '' },
...
async function validateAWSConfigRules() {
    try {
        ...
        try {
            if (testUserRule && testUserRule.Source.SourceIdentifier === 'S3_BUCKET_VERSIONING_ENABLED') {
                validationResult[1].weightage = 0.25;
                validationResult[1].status = true;
            } else {
                validationResult[1].error = "Assigned ConfigRule is not 'S3_BUCKET_VERSIONING_ENABLED'.";
            }
        } catch (error) {
            validationResult[1].error = "Assigned ConfigRule is not 'S3_BUCKET_VERSIONING_ENABLED'.";
        }
