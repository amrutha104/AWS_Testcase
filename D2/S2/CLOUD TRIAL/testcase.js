...
const result = [
    { name: "CloudTrail created in the name 'UserTrail'", weightage: 0, status: false, error: '' },
    { name: "Log Event Type is 'Data Events'", weightage: 0, status: false, error: '' },
    { name: "Data event type is 'Lambda'", weightage: 0, status: false, error: '' },
...
async function validateCloudTrailConfiguration() {
    ...
    try {
        const getTrailParams = { Name: 'UserTrail' };
        ...
        if (trailData) {
            ...
            try {
                const getEventSelectorsParams = { TrailName: 'UserTrail' };
                ...
                try {
                    if (eventSelectorsData && eventSelectorsData.AdvancedEventSelectors && eventSelectorsData.AdvancedEventSelectors.some(selector =>
                        selector.FieldSelectors && selector.FieldSelectors.some(fieldSelector =>
                            fieldSelector && fieldSelector.Equals && fieldSelector.Equals.includes('AWS::Lambda::Function')
                        )
                    )) {
                        ...
                        result[2].error = "Data event type is not 'Lambda'.";
                        result[3].error = "Log selector template does not have the expected configuration (Field: 'readOnly', Equals: 'true').";
                    }
                }
                catch (error) {
                    result[2].error = "Data event type is not 'Lambda'.";
                    result[3].error = "Data event type is not 'Lambda'.";
                }
            } catch (error) {
                result[1].error = "Log Event Type is not 'Data Events'.";
            ...
            result[0].error = "Trail 'UserTrail' not found.";
            result[1].error = "Trail 'UserTrail' not found.";
            result[2].error = "Trail 'UserTrail' not found.";
            result[3].error = "Trail 'UserTrail' not found.";
        }
        return result;
    } catch (error) {
